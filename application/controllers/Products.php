...
class Products extends CI_Controller
{
    ...
    public function index($CMID = null)
    {
        ...
        $consignees = $this->GetProducts->GetAllConsignee();
         $Products = $this->GetProducts->GetConsigneeProducts($CMID);
               ...
               // foreach($object->getWorksheetIterator() as $worksheet){
                    ...
                    for($row=2; $row<=$highestRow; $row++){
                        ...
                        for($col=0; $col<=$highestColumn; $col++){
                            ...
                            if(substr(trim($value),0,1) == "=") $value = $worksheet->getCellByColumnAndRow($col, $row)->getCalculatedValue();

                            
                            if($consignee == 46 && $col == 6){
