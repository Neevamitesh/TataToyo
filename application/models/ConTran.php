    ...
    class ConTran extends CI_Model{
        ...
        public function GetProducts($CMID,$UID){
           ...
           $this->db->join('ConsigneeMaster', 'ConsigneeMaster.CMID = ProductMaster.CMID AND ConsigneeMaster.isactive = 1');
            $this->db->join('ProductDetail', 'ProductDetail.PID = ProductMaster.id AND ProductMaster.isactive = 1 AND ProductDetail.KeyName = "BarcodeData"','Left');
        ...
        public function GetQty($CMID, $PID = null){   // 23112023
            if(!empty($PID)){
                return $this->db->query('SELECT *,c.CreatedAt DATE, Qty AS Maxi  FROM qtymaster qm 
                INNER JOIN CreateBatch c ON  qm.CBID = c.CBID
                WHERE qmid = 
                (
                    SELECT MAX(qmid) FROM qtymaster qm
                    INNER JOIN CreateBatch cb ON  qm.CBID = cb.CBID
                    INNER JOIN productmaster pm ON  pm.id = cb.PID
                    INNER JOIN productmaster oldProduct ON
                        oldProduct.ProductName = pm.ProductName AND
                        oldProduct.ProductCode = pm.ProductCode
                    WHERE qm.BitStatus = 1 AND cb.CMID = ? AND  PID IN (?,getOLDProductID(?))
                )',[
                $CMID,$PID,$PID
            ]);
            }else{
                return $this->db->query('
                    ...
                    (
                        SELECT max(qmid) FROM qtymaster qm
                        INNER JOIN CreateBatch cb on  qm.CBID = cb.CBID
                        WHERE qm.BitStatus = 1 AND cb.CMID = ?
                    )',[
                        $CMID
                    ]);
            }
        ...
        //     ]);
        // }
        public function GetAdditionalLabel($PrinterDpi,$CMID){
            if($PrinterDpi == "203 DPI"){
                $this->db->select("AdditionalLabel.ALID,AdditionalLabel.CMID,AdditionalLabel.PATH AS Path,AdditionalLabel.BitStatus,AdditionalLabel.CreatedBy,AdditionalLabel.CreatedAt");
                $this->db->from("AdditionalLabel");
                $this->db->where("BitStatus",1);
                $this->db->where("AdditionalLabel.CMID",$CMID);
                $query = $this->db->get();
                return $query;
            }elseif ($PrinterDpi == "300 DPI") {
                $this->db->select("AdditionalLabel.ALID,AdditionalLabel.CMID,AdditionalLabel.DpiPrnPath AS Path,AdditionalLabel.BitStatus,AdditionalLabel.CreatedBy,AdditionalLabel.CreatedAt");
                $this->db->from("AdditionalLabel");
                $this->db->where("BitStatus",1);
                $this->db->where("AdditionalLabel.CMID",$CMID);
                $query = $this->db->get();
        ...
        public function VerifyBarcode($Data){
            return $this->Api("P_VerifyBarcode",$Data);
        }
//221019
    function getShift(){
        $shift = null;
        $shiftData = $this->CT->shiftMaster('CurShift',null,null,null,null,date('Y-m-d H:i:s'));
        //  echo $this->db->last_query();
        if($shiftData->num_rows() > 0){
            $row = $shiftData->row();
            $shift = "{$row->ShiftTitle}";
        }
        return $shift;
    }
        public function shiftMaster($type = null,$shiftID = null,$shiftTitle = null,$shiftTime = null,$userID = null,$dateTime = null){
            $procName = 'STP_ShiftMaster';
            return $this->callProc($procName,[
                $type,$shiftID,$shiftTitle,$shiftTime,$userID,$dateTime
            ]);
        }
        public function STP_BatchReport($Data){
            return $this->Api("STP_BatchReport",$Data);
        }
    }
?>