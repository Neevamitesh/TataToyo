                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control form-control-rounded" required id="user" name="user">
                                        ...
                                        foreach ($consignees->result() as $loc) {
                                            ?>
                                            <option <?= $UID == $loc->CMID ? "selected" : ""; ?> value="<?= $loc->CMID; ?>"><?= $loc->consigneename . " (" . $loc->consigneecode . ")"; ?></option>
                                        <?php
                                    }
                                    ?>
                                    </select>
                                ...
                                if($isdata == 1){
                                    ...
                                    foreach($batches->result() as $batch){
                                        ?>
                                            <tr>
                                                <td><?= $c; ?></td>     
                                                <td><?= $batch->consigneename; ?></td>
                                                <td><?= $batch->productname; ?></td>
                                                <td><?= $batch->Quantity; ?></td>
                                                <td><?= date('d-m-Y h:i:s a', strtotime($batch->CreatedAt)); ?></td>
                                            </tr>
                                        <?php
                                        $c++;
                                    }
...
<script>
    $("#sortform").submit(function(e){
        ...
        window.location.href = "<?= base_url(); ?>Reports/BatchReport/" + uid+"/"+fdate+"/"+tdate;
    });
