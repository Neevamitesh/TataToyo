...
<!-- @@ start 26092019 -->
<?php
    $Flash = $this->session->flashdata('Report-Msg');
    if(isset($Flash)){
        ?>
            <h4 class="text-center text-danger temp-msg"><?= $Flash['Msg']?></h4>
        <?php
    }
?>
<form method="post" action="<?= base_url(); ?>Reports/Download" id="DownloadProduct" >
<div class="row">
        ...
        <div class="card">  
            ...
            <div class="card-body">
                <table>
                    ...
                    <td style="padding-left:2%;width:60%;">
                        
                            <select class="form-control form-control-rounded" onchange="GetData(this)" name="consignee" id="consignee" required>
                                <option value="" selected disabled>Select Consignee</option>
                                <?php
                                    foreach($consignees->result() as $con){
                                        ?>
                                            <option <?= $con->CMID == $CMID ? "selected" : ""; ?> value="<?= $con->CMID; ?>"><?= $con->consigneename." (".$con->consigneecode.")"." (".$con->LOC.")" ?></option>
                                        <?php
                                    }
                                ?>
                            </select>
                    </td>
                    <td id="tddownload" style="padding-left:20%;width:40%;">
                        
                        <button type="submit" class="btn btn-primary pull-right" title="Download" >Download</button>
                    </td>
                    </tr>
