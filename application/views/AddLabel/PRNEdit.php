...
<?php
    $Msg = $this->session->flashdata("Lab-File-Edit");
    if(!empty($Msg)){
        ?>
...
<div class="row">
    <div class="col-lg-8 mx-auto">
        <!-- <form action="<?= base_url("SpeLabel/AddPRNFile");?>" enctype="multipart/form-data" method="post"> -->
        <form action="<?= base_url("SpeLabel/UpdatePRNFile");?>" enctype="multipart/form-data" method="post">    
            <input type="hidden" name="Update" value="<?= md5('yo buddy !')?>">
            <div class="card">
                <div class="card-header gradient-forest">
                    <div class="card-title text-white">
                        Replace Additional Label File
                    </div>
                </div>
                <div class="card-body">
                    <div class="row">
                            ...
                            <select class="form-control form-control-rounded" name="Consignee" id="Consignee" required>
                                ...
                                <?php
                                    foreach ($OurConsignee->result() as $key => $row) {
                                        ?>
                                        <option value="<?= $row->CMID?>"><?= $row->consigneename." (".$row->LOC.")"?></option>
                                        <?php
                                    }
                                ?>
                            </select>
                    ...
                    <div class="row">
                        <div class="col-xl-6 col-12">
                            ...
                            <select class="form-control form-control-rounded" name="PrinterDpi" id="PrinterDpi" required>
