<?php
    class GetProducts extends CI_Model{
        function __construct(){
            ...
            $this->load->database();
            $this->load->helper('download');//@@26092019
        }
       ...
       public function GetAllProducts(){
        ...
        public function GetConsigneeProducts($CMID){
            $this->db->join("consigneemaster", "consigneemaster.CMID = productmaster.CMID");
           return $this->db->get_where("productmaster", ["productmaster.isactive" => 1,"consigneemaster.CMID" => $CMID]);
        }
        ...
        public function GetExcel(){
            $this->db->join("consigneemaster", "consigneemaster.CMID = productfiles.CMID");
           return $this->db->get_where("productfiles", ["productfiles.Bitstatus" => 1]);
        }

        public function GetAllConsignee(){
            $this->db->select("consigneemaster.*,cities.*,states.*,locationmaster.locationname LOC");
            $this->db->join("cities", "cities.city_id = consigneemaster.city", "left");
            $this->db->join("states", "states.state_id = consigneemaster.state", "left");
            $this->db->join("locationmaster", "locationmaster.LOCID = consigneemaster.location", "left");
            return $this->db->get_where("consigneemaster", ["locationmaster.isactive" => 1,"consigneemaster.isactive" => 1]);
        }
        //@@26092019 end
